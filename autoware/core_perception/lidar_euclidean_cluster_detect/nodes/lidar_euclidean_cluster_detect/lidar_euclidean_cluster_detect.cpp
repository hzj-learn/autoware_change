...
void publishDetectedObjects(const autoware_msgs::CloudClusterArray &in_clusters)
{
  ...
  //detected_objects.header.frame_id = "map";//note-tianyu add 将detect的object的frame_id转到地图坐标系下
                                        ...
                                        double in_max_cluster_distance = 0.5)
{
  ...
  // cluster
  // note-tianyu 聚类的完成的运算结果相当于是一个"嵌套的vector",里面存储的每一块cluster中的points的index
  for (auto it = cluster_indices.begin(); it != cluster_indices.end(); ++it)
  {
    ClusterPtr cluster(new Cluster());
                       ...
                       autoware_msgs::Centroids &in_out_centroids, autoware_msgs::CloudClusterArray &in_out_clusters)
{
  ...
  //note-sixu 这里可以决定是否多线程来进行欧式聚类，默认不使用
  ...
  } else
  {
    ...
    // note-tianyu 将pointcloud根据距离的远近分成五部分，分别进行聚类
    for (unsigned int i = 0; i < in_cloud_ptr->points.size(); i++)
    {
      pcl::PointXYZ current_point;
      current_point.x = in_cloud_ptr->points[i].x;
  ...
  //note-tianyu 对当前检测到的多种聚类进行check，可以融合的融合
  ...
  if (mid_clusters.size() > 0)
    ...
    // note-tianyu 得到融合后的final_clusters，遍历每一个cluster
                      ...
                      pcl::PointCloud<pcl::PointXYZ>::Ptr out_cloud_ptr, const double in_distance)
{
  ...
  void transformXYZIRCloud(const pcl::PointCloud<pcl::PointXYZ>& in_cloud,
...
/*
void velodyne_callback(const sensor_msgs::PointCloud2ConstPtr& raw_cloud) 
{
  ...
  pcl::fromROSMsg(*raw_cloud, *input_cloud_ptr);

  tf::TransformListener *tf_listener_ptr_;
  tf::StampedTransform transform;
  bool do_transform = false;

  PlannerHNS::ROSHelpers::GetTransformFromTF("map", raw_cloud->header.frame_id, transform);

 //if (do_transform)
 //{
    transformXYZIRCloud(*input_cloud_ptr, *transformed_cloud_ptr, transform);
    transformed_cloud_ptr->header.frame_id = "map";
 // }
 // else
 // { pcl::copyPointCloud(*input_cloud_ptr, *transformed_cloud_ptr);}

  sensor_msgs::PointCloud2::Ptr in_sensor_cloud;
  pcl::toROSMsg(*transformed_cloud_ptr, *in_sensor_cloud);
...
void velodyne_callback(const sensor_msgs::PointCloud2ConstPtr& in_sensor_cloud)
{
  ...
  if (!_using_sensor_cloud)
  {
    ...
    publishCloud(&_pub_points_lanes_cloud, nofloor_cloud_ptr);
    //note-tianyu 采用差分法线特征算法再对点云进行一次过滤
    if (_use_diffnormals)
      differenceNormalsSegmentation(nofloor_cloud_ptr, diffnormals_cloud_ptr);
    else
      diffnormals_cloud_ptr = nofloor_cloud_ptr;
    //note-tianyu 正式进入欧式聚类的接口，根据距离分块和感知障碍
    segmentByDistance(diffnormals_cloud_ptr, colored_clustered_cloud_ptr, centroids,
                      cloud_clusters);
