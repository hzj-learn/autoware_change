...
static void map_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  if (points_map_num != input->width)
  {
    ...
    //note-tianyu 大多数code和ndt_mapping是一样的
...
static void gnss_callback(const geometry_msgs::PoseStamped::ConstPtr& input)
{
  ...
  //note-tianyu 当这两种情况发生时，我们认为需要用gnss的pose来代替ndt优化计算出的current_pose
...
static void initialpose_callback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& input)
{
  ...
  {
    //note-tianyu 要注意我们从rviz手动给的pose是基于world坐标系的，所以需要将其转换到map坐标系
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  if (map_loaded == 1 && init_pos_set == 1)
  {
    ...
    //note-tianyu 这里也会计算ndt优化得出的pose和之前predict的pose之间的差值，如果差值很小，也会考虑使用predict的pose
    ...
    */
    ...
    //note-tianyu current_pose通过TF的方式来发布出去
