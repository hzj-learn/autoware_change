...
{
  initForROS();
  geo_.set_plane(plane_number_);//note-tianyu 根据设定的数字确定经度纬度高度转换的原点
}
...
void Nmea2TFPoseNode::convert(std::vector<std::string> nmea, ros::Time current_stamp)
{
  try
  {
    //note-tianyu 前两种字符串累计主要解析方向信息（nmea协议）
...
void Nmea2TFPoseNode::callbackFromNmeaSentence(const nmea_msgs::Sentence::ConstPtr &msg)
{
  ...
  // note-tianyu 当从gps中无法获取方向信息/出现了较长时间段内没有获得方向信息
...
std::vector<std::string> split(const std::string &string)
{
  ...
  //note-tianyu 以‘，’为间隔，将字符串分段并以vector形式来存储
