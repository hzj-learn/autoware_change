...
void LaneSelectNode::initForROS()
{
  ...
  //note-tianyu 下面三个在仿真场景下不订阅（这个涉及决策的内容--实现变道用的）
  ...
  sub6_ = nh_.subscribe("/decision_maker/state", 1, &LaneSelectNode::callbackFromDecisionMakerState, this);//类似于状态机的机制

  // setup publisher

  pub1_ = nh_.advertise<autoware_msgs::Lane>("base_waypoints", 1);//地图层执行的轨迹
  pub2_ = nh_.advertise<std_msgs::Int32>("closest_waypoint", 1);//阿克曼底盘用的（根据车辆的位姿、和base_waypoints计算出符合动力学的轨迹，核心）
  pub3_ = nh_.advertise<std_msgs::Int32>("change_flag", 1);           //是否变道
  pub4_ = nh_.advertise<std_msgs::Int32>("/current_lane_id", 1);
  pub5_ = nh_.advertise<autoware_msgs::VehicleLocation>("vehicle_location", 1);//定位的另一中表达形式
