...
//把vector_map的三个元素转换成lane_waypoint数据格式，并发布出来
void update_values()
{
  if (all_vmap.points.empty() || all_vmap.lanes.empty() || all_vmap.nodes.empty())//三个元素缺一不可
    return;
  //note-tianyu 读取输入的vector_map_info中的point，lane,stopline等元素，建立一个lane_vmap
  lane_vmap = lane_planner::vmap::create_lane_vmap(all_vmap, lane_planner::vmap::LNO_ALL);

//最终把vector_map的数据转成lane_waypoint格式
  autoware_msgs::LaneArray lane_waypoint;
  ...
  waypoint_pub.publish(lane_waypoint);//发布lane_waypoint
}
...
//发布多条全局路径/lane_waypoints_array（在订阅回调函数中实现）
  waypoint_pub = n.advertise<autoware_msgs::LaneArray>("/lane_waypoints_array", pub_waypoint_queue_size,
                 pub_waypoint_latch);

  ros::Subscriber route_sub = n.subscribe("/route_cmd", sub_route_queue_size, create_waypoint);//路线指令，实车调试需要，仿真环境可以简化
  ros::Subscriber point_sub = n.subscribe("/vector_map_info/point", sub_vmap_queue_size, cache_point);//订阅/vector_map中的点元素
  ros::Subscriber lane_sub = n.subscribe("/vector_map_info/lane", sub_vmap_queue_size, cache_lane);      //订阅/vector_map中的车道元素
  ros::Subscriber node_sub = n.subscribe("/vector_map_info/node", sub_vmap_queue_size, cache_node); //订阅/vector_map中的节点元素
