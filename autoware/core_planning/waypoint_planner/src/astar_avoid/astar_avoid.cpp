...
//订阅
  costmap_sub_ = nh_.subscribe("costmap", 1, &AstarAvoid::costmapCallback, this);//代价地图  
  current_pose_sub_ = nh_.subscribe("current_pose", 1, &AstarAvoid::currentPoseCallback, this);//当前定位
  current_velocity_sub_ = nh_.subscribe("current_velocity", 1, &AstarAvoid::currentVelocityCallback, this);//当前速度
  base_waypoints_sub_ = nh_.subscribe("base_waypoints", 1, &AstarAvoid::baseWaypointsCallback, this);//基础全局轨迹
  closest_waypoint_sub_ = nh_.subscribe("closest_waypoint", 1, &AstarAvoid::closestWaypointCallback, this);//符合动力学的轨迹
  obstacle_waypoint_sub_ = nh_.subscribe("obstacle_waypoint", 1, &AstarAvoid::obstacleWaypointCallback, this);//障碍物的点
...
{
  // check topics
  state_ = AstarAvoid::STATE::INITIALIZING;//设置算法参数的

  while (ros::ok())//等待所有的话题接收完毕
  {
    ros::spinOnce();
    if (checkInitialized())
    {
  ...
  obstacle_waypoint_index_ = -1;//默认前面没有障碍物

  // relaying mode at startup
  state_ = AstarAvoid::STATE::RELAYING;

  // start publish thread
  // note-sixu 额外启动的一个publish线程一直发路径点，不然后面的控制会有问题的
  ...
  while (ros::ok())
  {
    ...
    // avoidance mode
    bool found_obstacle = (obstacle_waypoint_index_ >= 0);//判断前面是否有障碍物，-1表示没有
    bool avoid_velocity = (current_velocity_.twist.linear.x < avoid_start_velocity_ / 3.6);//设置避障的速度

    // update state
    // note-tianyu 在避让过程中的几种状态
    if (state_ == AstarAvoid::STATE::RELAYING)//正常行驶，没有障碍物
    {
    ...
    else if (state_ == AstarAvoid::STATE::STOPPING)//车前出现障碍物，但是还没进行规划，要先停下来
    {
      bool replan = ((ros::WallTime::now() - start_plan_time).toSec() > replan_interval_);

      if (!found_obstacle)//障碍物一直存在在开始规划
      {
    ...
    else if (state_ == AstarAvoid::STATE::PLANNING)//开始规划
    {
      start_plan_time = ros::WallTime::now();

      if (planAvoidWaypoints(end_of_avoid_index))//由此进入astar planning，end_of_avoid_index是规划goal的点
      {//找到path
    ...
    else if (state_ == AstarAvoid::STATE::AVOIDING)//开始避障行驶
    {
...
bool AstarAvoid::planAvoidWaypoints(int& end_of_avoid_index)
{
  ...
  for (int i = search_waypoints_delta_; i < static_cast<int>(search_waypoints_size_); i += search_waypoints_delta_)
  {
    ...
    if (found_path)
    {
      ...
      //将avoidwaypoints的路径merge到主路径【轨迹拼接的过程】
