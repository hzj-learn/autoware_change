...
bool AstarSearch::setStartNode(const geometry_msgs::Pose& start_pose)
{
  ...
  poseToIndex(start_pose_local_.pose, &index_x, &index_y, &index_theta);//pose的位置转成index位置表示
...
bool AstarSearch::search()
{
 ...
 // openlist为空，搜索失败
  while (!openlist_.empty())
  {
    ...
    // note-tianyu 从openlist中选择cost最小的值的序号，拿掉后就删除了
