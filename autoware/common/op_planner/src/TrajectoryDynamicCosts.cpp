    ...
    const std::vector<PlannerHNS::DetectedObject>& obj_list, const int& iCurrentIndex)
{
  ...
  //note-tianyu 将存储感知障碍物信息的obj_list转存到m_AllContourPoints中
  ...
  //note-tianyu 计算collision权重 主要分为lateral_cost 和 longitudinal_cost，前者代表局部轨迹距离障碍物的水平距离，后者代表局部轨迹距离最近障碍物的垂直距离
    ...
    const CAR_BASIC_INFO& carInfo, const VehicleState& vehicleState)
{
  ...
  {
    std::cout<<totalPaths[i].pos.x<<" , "<<totalPaths[i].pos.y<<" , "<<totalPaths[i].pos.z<<std::endl;
  }

  for(int i = 0; i<contourPoints.size(); i++)
  {
    std::cout<<contourPoints[i].pos.x<<","<<contourPoints[i].pos.y<<","<<contourPoints[i].pos.z<<std::endl;
  }
  ...
  if(rollOuts.size() > 0 && rollOuts.at(0).size()>0)
  {
    ...
    for(unsigned int it=0; it< rollOuts.size(); it++)
    {
      ...
      for(unsigned int icon = 0; icon < contourPoints.size(); icon++)
      {
        ...
        //std::cout<<"cur_pose: "<<currState.pos.x<<","<<currState.pos.y<<","<<currState.pos.z<<std::endl;
        //std::cout<<"contourPoint: "<<contourPoints.at(icon).pos.x<<","<<contourPoints.at(icon).pos.y<<","<<contourPoints.at(icon).pos.z<<std::endl;
...
//          if(close_in_percentage <= 0 || close_in_percentage > 1) close_in_percentage = 1;
        ...
        //note-tianyu 如果计算出的车辆和障碍之间的横纵向距离，不在可能发生碰撞的范围内，则直接continue掉
