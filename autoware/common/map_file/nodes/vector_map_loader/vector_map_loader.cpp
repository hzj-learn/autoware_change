...
U createObjectArray(const std::string& file_path)
{
  ...
  obj_array.data = vector_map::parse<T>(file_path);//note-tianyu 通过这个函数来解析读取csv文件中的元素信息,返回一个array结构的变量
  return obj_array;
}



// note-tianyu 下列create Marker函数都大同小异
...
int main(int argc, char **argv)
{
  ...
  //note-tianyu 完成csv文件的解析后将各种矢量信息发布出去。
  ...
  else //note-tianyu 执行map launch文件会从这个分支来进行执行
  {
  ...
  for (const auto& file_path : file_paths)//读取各种vector map的csv文件
  {
    ...
    {
      //note-tianyu 读取"crosswalk.csv"文件中数据,创建一个ObjectArray，并且通过cross_walk_pub发布
